...
(function () {
  ...
  function defaultState() {
    return {
      ...
      muted: false,             // 声音偏好
      volume: 0.5,              // 音量 0~1
      soundType: 'auto',        // 'auto'|'wind'|'rain'|'stream'|'birds'|'crickets'
    };
  }
  function load() {
    try {
  ...
  /* ---------------- 声音系统（多环境音 + 昼夜差异 + 音量可调） ---------------- */
  // state 增加: muted, volume(0~1), soundType('auto'|'wind'|'rain'|'stream'|'birds'|'crickets')
  let audioCtx = null;
  let activeNodes = [];   // 当前活跃的音频节点（统一管理）
  let soundOn = false;

  // 音量控制
  let masterGain = null;
  function getVolume() { return state.volume != null ? state.volume : 0.5; }
  ...
  function startAmbient() {
    ...
    try {
      ...
      if (type === 'rain') {
        nodes.push(makeRain(audioCtx, masterGain));
      } else if (type === 'stream') {
        nodes.push(makeStream(audioCtx, masterGain));
      } else if (type === 'birds') {
        nodes.push(makeBirds(audioCtx, masterGain));
      } else if (type === 'crickets') {
        nodes.push(makeCrickets(audioCtx, masterGain));
      }
      activeNodes = nodes;
      soundOn = true;
  ...
  function setVolume(v) {
    ...
    // 音量到 0 自动静音
    if (v === 0 && !state.muted) setMuted(true);
    else if (v > 0 && state.muted) setMuted(false);
  }
  ...
  function startBreathAmbient() {
    ...
    try {
      ...
      breathAudioNodes = [{ src: osc, gain: g }, { stopped: false, src: { stop: () => { stopped = true; } } }];
      breathAudioNodes[1]._bell = bell;
      breathAudioNodes[1].setStopped = (v) => { stopped = v; };
    } catch(e){}
  }
  function stopBreathAmbient() {
  ...
  function bindEvents() {
    ...
    [$('breathOverlay'), $('feelOverlay'), $('soundOverlay')].forEach(ov => {
      ov.addEventListener('click', (e) => {
        ...
        if (ov === $('breathOverlay')) { closeBreath(); stopBreathAmbient(); }
        else if (ov === $('soundOverlay')) { hideOverlay(ov); }
        else hideOverlay(ov);
      });
    });
    // 离开/隐藏自动保存
