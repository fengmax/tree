...
function makeBirds(ctx, gainNode) {
  ...
  function chirp() {
    ...
    env.gain.linearRampToValueAtTime(0.14, ctx.currentTime + 0.02);
...
export function playWaterSound() {
  ...
  try {
    ...
    g.gain.linearRampToValueAtTime(0.22 * getVolume(), t + 0.08);
    ...
    for (let i = 0; i < drops; i++) {
      ...
      env.gain.linearRampToValueAtTime(0.09 * getVolume(), dt + 0.005);
...
export function startBreathAmbient() {
  ...
  try {
    ...
    osc.start();
    g.gain.setTargetAtTime(0.045, audioCtx.currentTime, 1.5);
    let stopped = false;
    function bell() {
      if (stopped || !audioCtx) return;
      const t = audioCtx.currentTime;
      ...
      const env = audioCtx.createGain();
      env.gain.setValueAtTime(0, t);
      env.gain.linearRampToValueAtTime(0.022 * getVolume(), t + 0.02);
